...
<?php if (($authuser['id'] === $bidinfo->user_id) ||  ($authuser['id'] === $bidinfo->biditem->user_id)) : ?>
	...
	<fieldset>
		...
		<?php if ($bidinfo->status == 0 && $authuser['id'] === $bidinfo->user_id) : ?>
			...
			<!-- 出品者 -->
		<?php elseif ($bidinfo->status == 0 && $authuser['id'] === $bidinfo->biditem->user_id) : ?>
			<h5>発送先住所</h5>
		...
		<?php if ($bidinfo->status == 1 && $authuser['id']  == $bidinfo->biditem->user_id) : ?>
			<h5>発送連絡</h5>
			<p><?= __('発送した？') ?></p>
			<?php echo $this->Form->button('status', ['type' => 'post', 'value' => 2]); ?>
			<!-- statusが落札済み、発済みでユーザーが出品者なら -->
		<?php elseif ($bidinfo->status == 2 && $authuser['id']  == $bidinfo->biditem->user_id) : ?>
			<h5>発送連絡</h5>
			<p>※受け取りをお待ちください</p>
			<!-- 落札者	 -->
		...
		<?php elseif ($bidinfo->status == 3 && $authuser['id']  == $bidinfo->biditem->user_id) : ?>
